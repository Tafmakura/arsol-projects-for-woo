...
(function($) {
    ...
    var ArsolProposalQuotation = {
        ...
        setInitialAverageTotal: function() {
            ...
            var initialTotal = arsol_pfw_proposal_quotation_vars.average_monthly_total_formatted;
            if (initialTotal) {
...
(function($) {
    ...
    var ArsolProposalQuotation = {
        ...
        getDailyCost: function(price, interval, period) {
            ...
            interval = parseInt(interval) || 1;
            var days_in_period = 0;
            var constants = arsol_pfw_quotation_vars.calculation_constants;
        ...
        setInitialAverageTotal: function() {
            ...
            // but we'll keep it here in case it's needed later.
            // It's called on init but does nothing if the localized var isn't there.
            var initialTotal = arsol_pfw_quotation_vars.average_monthly_total_formatted;
            if (initialTotal) {
                $('#average-monthly-total-display').html(initialTotal);
            }
        },
        ...
        loadExistingItems: function() {
            var self = this;
            var items = arsol_pfw_quotation_vars.line_items;
        ...
        initSelect2: function($row) {
            $row.find('.arsol-description-input').select2({
                ajax: {
                    url: arsol_pfw_quotation_vars.ajax_url,
                    dataType: 'json',
                    delay: 250,
                    data: function(params) {
                        return {
                            action: 'arsol_proposal_quotation_ajax_search_products',
                            nonce: arsol_pfw_quotation_vars.nonce,
                            search: params.term,
                        };
                    },
                    processResults: function(data) { return { results: data.data }; },
        ...
        formatPrice: function(price) {
            var currencySymbol = arsol_pfw_quotation_vars.currency_symbol;
            var formattedPrice = Number(price).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
            return '<span class="woocommerce-Price-amount amount"><bdi><span class="woocommerce-Price-currencySymbol">' + currencySymbol + '</span>' + formattedPrice + '</bdi></span>';
        },

        // Remove the total storage functionality since we're not storing totals anymore
        storeCalculatedTotals: function(oneTimeTotal, recurringTotals) {
            // No longer storing totals - removed completely
            // This was the source of the mismatch between form fields and PHP processing
        }
    };
