...
if (!$post || ($post->post_type !== 'arsol-pfw-proposal' && $post->post_type !== 'arsol-pfw-project')) {
    return;
}

$post_id = $post->ID;

// Get original request data based on post type
if ($post->post_type === 'arsol-pfw-proposal') {
    $original_request_id = get_post_meta($post_id, '_arsol_pfw_proposal_request_id', true);
    $original_request_title = get_post_meta($post_id, '_arsol_pfw_proposal_request_title', true);
    $original_request_content = get_post_meta($post_id, '_arsol_pfw_proposal_request_details', true);
    ...
    $original_request_budget = get_post_meta($post_id, '_arsol_pfw_project_request_budget', true);
    $original_request_start_date = get_post_meta($post_id, '_arsol_pfw_project_request_start_date', true);
    $original_request_delivery_date = get_post_meta($post_id, '_arsol_pfw_project_request_delivery_date', true);
}
...
<?php if ($has_original_data): ?>
    ...
    <p class="form-field form-field-wide">
        <label><strong><?php _e('Budget:', 'arsol-pfw'); ?></strong></label>
        <?php 
    ...
    <p class="form-field form-field-wide">
        <label><strong><?php _e('Details:', 'arsol-pfw'); ?></strong></label>
...
<?php else: ?>
    <p><?php 
    if ($post->post_type === 'arsol-pfw-proposal') {
        _e('This proposal was created directly without an initial customer request.', 'arsol-pfw');
    } else {
        _e('This project was created directly without an initial customer request.', 'arsol-pfw');
    }
    ?></p>
<?php endif; ?> 