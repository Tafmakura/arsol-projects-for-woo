...
class Project {
    ...
    public function render_project_details_meta_box($post) {
        ...
        // Get statuses, excluding 'not-started'
        $statuses = get_terms(array(
            'taxonomy'   => 'arsol-project-status',
            'hide_empty' => false,
            'exclude'    => get_term_by('slug', 'not-started', 'arsol-project-status')->term_id ?? 0,
        ));
            ...
            <p>
                ...
                <select name="project_status" id="project_status" style="width:100%">
                    <?php if ($current_status === 'not-started') : ?>
                        <option value="not-started" selected="selected"><?php _e('Not Started', 'arsol-pfw'); ?></option>
                    <?php endif; ?>
