...
class Proposal {
    ...
    public function render_proposal_details_meta_box($post) {
        ...
        // Get original request data (if converted from request)
    ...
    public function save_proposal_details($post_id) {
        ...
        if ($cost_proposal_type === 'budget') {
            ...
            // Save budget details
            if (isset($_POST['arsol_pfw_proposal_budget_onetime_details'])) {
                update_post_meta($post_id, '_arsol_pfw_proposal_budget_onetime_details', sanitize_text_field($_POST['arsol_pfw_proposal_budget_onetime_details']));
            }
            ...
            if (isset($_POST['arsol_pfw_proposal_budget_recurring_details'])) {
                update_post_meta($post_id, '_arsol_pfw_proposal_budget_recurring_details', sanitize_text_field($_POST['arsol_pfw_proposal_budget_recurring_details']));
        }
            ...
            if (!empty($_POST['arsol_pfw_proposal_budget_recurring_amount']) && $_POST['arsol_pfw_proposal_budget_recurring_amount'] > 0) {
                if (isset($_POST['arsol_pfw_proposal_budget_recurring_billing_interval'])) {
                    update_post_meta($post_id, '_arsol_pfw_proposal_budget_recurring_billing_interval', sanitize_text_field($_POST['arsol_pfw_proposal_budget_recurring_billing_interval']));
                }
                if (isset($_POST['arsol_pfw_proposal_budget_recurring_billing_period'])) {
                    update_post_meta($post_id, '_arsol_pfw_proposal_budget_recurring_billing_period', sanitize_text_field($_POST['arsol_pfw_proposal_budget_recurring_billing_period']));
                }
        ...
        if ($cost_proposal_type !== 'quotation') {
             ...
             delete_post_meta($post_id, '_arsol_pfw_proposal_quotation_recurring_totals');
        }
    ...
    private function validate_budget_fields($post_id) {
        ...
        if ($recurring_budget_amount > 0) {
            ...
            $billing_interval = isset($_POST['arsol_pfw_proposal_budget_recurring_billing_interval']) ? sanitize_text_field($_POST['arsol_pfw_proposal_budget_recurring_billing_interval']) : '';
            $billing_period = isset($_POST['arsol_pfw_proposal_budget_recurring_billing_period']) ? sanitize_text_field($_POST['arsol_pfw_proposal_budget_recurring_billing_period']) : '';
