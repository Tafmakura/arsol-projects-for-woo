...
namespace Arsol_Projects_For_Woo\Custom_Post_Types\Project\Core;
...
class Setup {
    ...
    public function register_post_type() {
        ...
        if (\Arsol_Projects_For_Woo\Admin\Settings\General::is_comments_enabled_for_post_type('arsol-pfw-project')) {
            $supports[] = 'comments';
        }
    ...
    public function handle_project_template_redirect() {
        if (is_singular('arsol-pfw-project')) {
            ...
            if (!\Arsol_Projects_For_Woo\Core\Permissions::user_can_view_project($user_id, $project_id)) {
    ...
    public function render_project_header_container() {
        ...
        $project = new \Arsol_Projects_For_Woo\Custom_Post_Types\Arsol_PFW_Project($post->ID);
        
        $template_path = ARSOL_PFW_PLUGIN_DIR . 'includes/ui/components/admin/section-edit-project-header.php';
