...
class List_Controller {
    ...
    private function render_stage_column_direct($post_id) {
        $terms = wp_get_object_terms($post_id, 'arsol-pfw-proposal-stage', ['fields' => 'all']);
    ...
    /**
     * Render project column using direct WordPress functions
     */
    private function render_project_column_direct($post_id) {
        $project_id = get_post_meta($post_id, '_arsol_pfw_parent_project_id', true);
        if ($project_id) {
    ...
    public function add_filters() {
        ...
        if ($typenow === 'arsol-pfw-proposal') {
            ...
            echo '<select name="customer" id="filter-by-customer" class="wc-customer-search" data-placeholder="' . esc_attr__('Filter by customer', 'arsol-pfw') . '" data-allow_clear="true" data-action="arsol_pfw_ajax_search_users" data-search-type="customers" data-security="' . esc_attr(wp_create_nonce('search-users')) . '">';
