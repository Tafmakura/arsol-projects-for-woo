 ...
 *
 * Provides OOP methods for managing proposal stages with WooCommerce compatibility
 */
class Proposal_Stage implements Stage_Interface, WooCommerce_Stage_Interface, Static_Stage_Interface
{
    ...
    private function ensure_default_stage_terms(): void
    {
        ...
        if (!taxonomy_exists('arsol-pfw-proposal-stage')) {
            return;
        }
    ...
    public function set_stage(string $stage): bool
    {
        ...
        do_action('arsol_pfw_proposal_stage_changed', $this->proposal_id, $old_stage, $stage);
        do_action("arsol_pfw_proposal_stage_{$stage}", $this->proposal_id);
        do_action("arsol_pfw_proposal_stage_{$old_stage}_to_{$stage}", $this->proposal_id);
        return true;
    }
    ...
    {
        $order_id = get_post_meta($this->proposal_id, '_arsol_pfw_order_id', true);
        if (!$order_id) {
            return false;
        }
    ...
    private function get_stage_start_time(): int
    {
        if (empty($this->history)) {
            return get_post_time('U', true, $this->proposal_id);
        }
