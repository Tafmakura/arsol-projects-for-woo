...
class Frontend_Template_Sidebar_Meta {
    ...
    private function get_proposal_metadata($post_id, $status) {
        ...
        // Budget
        $budget_data = $this->get_proposal_budget_data($post_id);
        if (!empty($budget_data['onetime_amount'])) {
            $metadata['budget'] = array(
                'label' => __('Budget', 'arsol-pfw'),
                'value' => wc_price($budget_data['onetime_amount'], array('currency' => $budget_data['currency'])),
                'type' => 'currency'
            );
        }
    ...
    public function get_proposal_budget_data($post_id) {
        ...
        return array(
            ...
            'onetime_amount' => !empty($onetime_data['amount']) ? $onetime_data['amount'] : 0,
            'onetime_details' => !empty($onetime_data['details']) ? $onetime_data['details'] : '',
            'recurring_amount' => !empty($recurring_data['amount']) ? $recurring_data['amount'] : 0,
            'recurring_details' => !empty($recurring_data['details']) ? $recurring_data['details'] : '',
            'billing_interval' => !empty($recurring_data['billing_interval']) ? $recurring_data['billing_interval'] : '',
            'billing_period' => !empty($recurring_data['billing_period']) ? $recurring_data['billing_period'] : '',
            'start_date' => !empty($recurring_data['start_date']) ? $recurring_data['start_date'] : '',
            'currency' => !empty($onetime_data['currency']) ? $onetime_data['currency'] : get_woocommerce_currency()
        );
    }
}
