...
function arsol_debug_page() {
    ...
    <div class="wrap">
        ...
        <?php if (isset($_POST['assign_caps_manually']) && wp_verify_nonce($_POST['_wpnonce'], 'arsol_assign_caps')): ?>
            <?php
            $admin_role = get_role('administrator');
            if ($admin_role) {
                // Use the EXACT same capabilities as the plugin's settings system
        ...
        <form method="post" style="margin: 20px 0;">
            <?php wp_nonce_field('arsol_assign_caps'); ?>
        ...
        <h2>🔧 Settings-Based Assignment (Recommended)</h2>
        <p>OR trigger the settings-based assignment directly:</p>
        
        <?php if (isset($_POST['trigger_settings_assignment']) && wp_verify_nonce($_POST['_wpnonce'], 'arsol_trigger_settings')): ?>
            <?php
            // Get current settings
            $settings = get_option('arsol_pfw_general_settings', array());
            
            // Ensure administrator is in both manager and user roles
            $settings['project_manager_roles'] = isset($settings['project_manager_roles']) ? $settings['project_manager_roles'] : array();
            $settings['project_user_roles'] = isset($settings['project_user_roles']) ? $settings['project_user_roles'] : array();
            
            if (!in_array('administrator', $settings['project_manager_roles'])) {
                $settings['project_manager_roles'][] = 'administrator';
            }
            if (!in_array('administrator', $settings['project_user_roles'])) {
                $settings['project_user_roles'][] = 'administrator';
            }
            
            // Save settings to trigger capability assignment
            update_option('arsol_pfw_general_settings', $settings);
            
            echo '<div class="notice notice-success"><p><strong>✅ Settings updated and capabilities assigned through the plugin system! Refresh this page to see the changes.</strong></p></div>';
            ?>
        <?php endif; ?>
        
        <form method="post" style="margin: 20px 0;">
            <?php wp_nonce_field('arsol_trigger_settings'); ?>
            <button type="submit" name="trigger_settings_assignment" class="button button-secondary">
                ⚙️ Trigger Settings-Based Assignment
            </button>
        </form>
